...
class BaseVAE(pl.LightningModule):
    ...
    def forward(self, *inputs: Tensor, validation: bool = False, m: Optional[float] = None, M: Optional[float] = None):
        ...
        # print(loss, reconstruction_loss, kl_loss, metric_loss)
        loss = 0 
        if metric_loss !=0:
            if (not torch.isnan(metric_loss)) and (not torch.isinf(metric_loss)):
                loss +=self.beta_metric_loss * metric_loss
        if (not torch.isnan(reconstruction_loss)) and (not torch.isinf(reconstruction_loss)):
            loss +=reconstruction_loss
        if (not torch.isnan(kl_loss)) and (not torch.isinf(kl_loss)):
            loss +=beta * kl_loss
        if prediction_loss!=0:
            if (not torch.isnan(prediction_loss)) and (not torch.isinf(prediction_loss)):
                loss +=self.beta_target_pred_loss * prediction_loss
