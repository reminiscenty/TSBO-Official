...
from teacher_student_scripts.tsbo_train import setup_args, train_teacher_student, add_tsbo_args, add_tsparams_to_path
...
def get_root_path(lso_strategy: str, weight_type, k, r,
    ...
    #     if random_sample:
    #         result_path +='-gaussian_sample'
    #     else:
    #         result_path += '-gev_sample'
    # if unlabel_retrain:
    #     result_path +='-unlabel_retrain'
    #     if unlabel_retrain_type =='lhs':
    #         result_path += '-random_sample'
    #     elif unlabel_retrain_type =='gev':
    #         result_path += '-gev_sample'
    #     elif unlabel_retrain_type == 'hybrid':
    #         result_path += '-hybrid_sample'
    # if self_training or unlabel_retrain:
    #     result_path+='-topk_'+str(unlabel_topk)
    return result_path
...
def get_path(lso_strategy: str, weight_type, k, r, predict_target,
             ...
             estimate_rec_error: bool, cost_aware_gamma_sched: Optional[str], use_binary_data: bool,
             semi_supervised: Optional[bool], args):
    """ Get result path
...
def main_aux(args, result_dir):
        ...
        for ret_idx in range(start_num_retrain, start_num_retrain + num_retrain):
            ...
            if args.lso_strategy == "opt":
                ...
                x_new, y_new = latent_turbo(
                    ...
                    num_queries_to_do=num_queries_to_do, 
                    use_binary_data=args.use_binary_data, 
                    gp_data_file=os.path.join(result_dir, f"train_data_iter{samples_so_far + num_queries_to_do}"),
                )
                rand_point_due_bo_fail = [0] * num_queries_to_do
            else:
    ...
    new_dir = os.path.join(ROOT_PROJECT,"results/topology",dir[-2],dir[-1])
...
def latent_optimization(
    ...
    while len(all_new_imgs) < num_queries_to_do:
        ...
        z_opt = unnormalize(z_opt.detach(), bounds)
        ...
        all_new_score_noise = []
        if semi_supervised:
            checks = [1 if (new_img == img).all() else 0 for img in datamodule.data_train[train_inds]]
        else:
            checks = [1 if (new_img == img).all() else 0 for img in datamodule.data_train]
        checks += [1 if (new_img == img).all() else 0 for img in all_img_noise]
        seen = sum(checks) > 0
        print(seen)
        if seen :
            print('seen image, preturb z_opt 100 times')
            ...
            for random_trials in range(100):
                ...
                if new_img_noise is None:
                    ...
                    if semi_supervised:
                        checks_noise = [1 if (new_img_noise == img).all() else 0 for img in datamodule.data_train[train_inds]]
                    else:
                        checks_noise = [1 if (new_img_noise == img).all() else 0 for img in datamodule.data_train]
                    checks_noise += [1 if (new_img_noise == img).all() else 0 for img in all_img_noise]
        ...
        all_new_score_noise = np.array(all_new_score_noise).flatten().astype(float)
        ...
        if do_robust and estimate_rec_error:
            ...
            new_errors = None
        print('num of x and new z:',len(X_train), len(all_new_z_opt_noise))
...
def latent_turbo(model, 
    ...
    X_train, y_train, X_test, y_test, X_mean, y_mean, X_std, y_std, train_inds, test_inds = get_latent_encodings(
        use_test_set=False,
        use_full_data_for_gp=False,
        model=model,
        data_file=gp_data_file,
        data_imgs=datamodule.data_train,
        data_scores=datamodule.prop_train,
        n_best=args.n_best_points,
        n_rand=args.n_rand_points,
        tkwargs=tkwargs,
        return_inds=True
    )
    # bounds = torch.zeros(2, X_train.shape[1], **tkwargs)
    ...
    lb = X_train.min(0)
    ub = X_train.max(0)
    # assert False, print(X_train.shape, lb.shape, ub.shape)

    """ Draws samples from latent space and appends to the dataset """
    target = np.load(get_topology_binary_target_path()) if use_binary_data else np.load(get_topology_target_path())
    s = ScoreFunction(model, target, tkwargs)
    turbo1 = Turbo1(
    ...
    return model.decode_deterministic(z=torch.atleast_2d(torch.from_numpy(turbo1.X).to(**tkwargs))).detach().squeeze(0).cpu().numpy().reshape(-1, 40, 40), -turbo1.fX.reshape(-1)

class ScoreFunction():
    def __init__(self, model, target, tkwargs) -> None:
        super().__init__()
        self.target = target
        self.model = model
        self.tkwargs = tkwargs
    def score_function(self, z, metric='cos'):
        z = torch.atleast_2d(torch.from_numpy(z).to(**self.tkwargs))
        predicted_image = self.model.decode_deterministic(z=z).detach().squeeze(0).cpu().numpy()

        # predicted_image = _decode_z_props(model=self.model,
        #                                     z=torch.from_numpy(new_latents).view(1,-1).to(**self.tkwargs),
        #                                     target=np.load(get_topology_binary_target_path()),
        ...
        if isinstance(target, Tensor):
            target = target.detach().cpu().to(torch.float)
        ...
        if metric == 'cos':
            ...
            print(cosine_sim(pred_flat, target_flat).reshape(-1).cpu().numpy()[0])
            return -cosine_sim(pred_flat, target_flat).reshape(-1).cpu().numpy()[0]
        elif metric == 'jaccard':
            return sp.spatial.distance.jaccard(predicted_image.flatten(), target.flatten())
        else:
