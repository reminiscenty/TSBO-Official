...
def main_aux(args, result_dir):
    ...
    new_dir = os.path.join(ROOT_PROJECT,"results/expr",dir[-2],dir[-1])
    ...
    while len(all_new_exprs) < num_queries_to_do:
        ...
        try:
            if args.self_training:
                ...
                y_train_std_arg = y_train_arg.add(-y_train_arg.mean()).div(y_train_arg.std())
                gp_model = gp_torch_train(
                    train_x=train_x_arg,
                    train_y=-y_train_std_arg,  # minimize
            ...
            if args.self_training:
                unlabel_x, unlabel_y = train_teacher_student(args, X_train, -y_train_std, X_train, -y_train_std, bounds, datamodule)
                train_x_arg = torch.cat((train_x, unlabel_x),dim=0)
                y_train_std_arg = torch.cat((y_train_std, -unlabel_y),dim=0)
                gp_model = gp_torch_train(
                    train_x=train_x,
                    train_y=-y_train_std,  # minimize
        ...
        print('----------------------')
        print(X_train.shape[0], train_x.shape[0], error_train.shape[0], len(train_inds))
        print('estimate_rec_error:',estimate_rec_error,' init:',init, ' current_n_inducing_points:',current_n_inducing_points)
        print('----------------------')
        if do_robust:
            if (estimate_rec_error or init) and error_train.shape[0]== train_x.shape[0]:
        ...
        all_new_expr_noise = []
        all_new_score_noise = []
        if new_expr in datamodule.expr_train[train_inds]:
            ...
            for random_trials in range(100):
                ...
                if new_expr_noise is None or new_expr_noise.item() is None:
                    ...
                    if new_expr_noise in datamodule.expr_train[train_inds]:
...
def latent_turbo(model, 
    ...
    return model.decode_from_latent_space(zs=torch.atleast_2d(torch.from_numpy(turbo1.X).to(**tkwargs)),n_decode_attempts=20), turbo1.fX.reshape(-1)
...
class ScoreFunction():
    ...
    def score_function(self, z, target_eq='1 / 3 + x + sin( x * x )', worst=7.0):
        """ compute equation scores of given inputs """
        z = torch.atleast_2d(torch.from_numpy(z).to(**self.tkwargs))
        input = self.model.decode_from_latent_space(zs=z, n_decode_attempts=20)
        # define inputs and outputs of ground truth target expression
        x = np.linspace(-10, 10, 1000)
        # print('zzzzzzzzzzzzzz',target_eq)
        try:
        ...
        try:
            ...
            score = np.min((worst, np.log(1 + np.mean((y_pred - yT) ** 2))))
        except:
            score = worst
