...
if ((semi_supervised == 0)); then query_budget=500; else query_budget=200; fi
...
unlabel_loss_ratio='--unlabel_loss_ratio=0.001'
unlabel_loss='--unlabel_loss=MSE'
unlabel_gauss_var='--unlabel_gauss_var=0.1'
...
for seed in "${seed_array[@]}"; do
  ...
  cmd="python weighted_retraining/weighted_retraining/robust_opt_scripts/robust_opt_topology.py \
          ...
          $unlabel_loss_ratio $unlabel_loss $joint_train_epochs \
          $unlabel_sample_type $gaussian_lr $student_fit_lr $ts_version $unlabel_gauss_var"
  $cmd
  echo $cmd
done