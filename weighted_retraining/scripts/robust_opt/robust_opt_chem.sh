...
metric_loss_kws=("" "{'threshold':.05,'hard':True}" "{'threshold':.1}" "{'threshold':.1,'soft':True}" "{'threshold':.2,'soft':True}" "{'threshold':.1,'soft':True,'eta':0.05}" "{}")
...
semi_supervised=1
n_init_bo_points=200
# n_init_bo_points=2500
...
if ((semi_supervised == 0)); then query_budget=250; else query_budget=250; fi
...
#-- Choose whether to use self training method#
self_training=1
if ((self_training == 0)); then self_training=''; else self_training='--self_training'; fi
topk=30
unlabel_sample_type='--unlabel_sample_type=gev'
unlabel_sample_type='--unlabel_sample_type=gaussian'
...
# validation_type='--validation_type=labelall'
# validation_type='--validation_type=random'
# is_validation='--is_validation'


ts_version='--ts_version=4'


# is_additional_noise='--is_additional_noise'
is_additional_noise=''
# additional_noise_std='--additional_noise_std=1.0'
...
lso_strategy_ind=0
lso_strategies=("opt" "sample" "turbo")
...
for seed in "${seed_array[@]}"; do
  ...
  cmd="python ./weighted_retraining/weighted_retraining/robust_opt_scripts/robust_opt_chem.py \
            ...
            $self_training  --topk=$topk \
            $unlabel_sample_type $gaussian_lr $student_fit_lr $unlabel_loss_ratio $unlabel_loss $joint_train_epochs
            $validation_type $is_validation  $ts_version 
            $is_additional_noise $additional_noise_std"
  $cmd
  echo $cmd
