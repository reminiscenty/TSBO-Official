...
# k='inf' # factor for weighted retraining
...
#-- Choose the metric loss you want to use -- (default contrastive: 1, default triplet: 5 8) --#
metric_loss_ind=5
# metric_loss_ind=0
...
metric_loss_kws=("" "{'threshold':.1}" "{'threshold':.1,'hard':True}" "{'threshold':.05,'hard':True}"
  "{'threshold':.2,'hard':True}" "{'threshold':.1,'soft':True}" "{'threshold':.1,'soft':True}"
...
if ((semi_supervised == 0)); then query_budget=500; else query_budget=250; fi
...
lso_strategy_ind=0
lso_strategies=("opt" "random_search" "turbo")
...
for seed in "${seed_array[@]}"; do
  ...
  cmd="python weighted_retraining/weighted_retraining/robust_opt_scripts/robust_opt_expr.py \
        ...
        --latent_dim $latent_dim $semi_supervised $n_init_bo_points --lr $lr $use_pretrained\
